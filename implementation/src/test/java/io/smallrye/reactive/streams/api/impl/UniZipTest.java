...
public class UniZipTest {
    ...
    public void testWithTwoOneFailure() {
        Uni<Integer> uni = Uni.from().value(1);
        AssertSubscriber<Pair<Integer, Integer>> subscriber =
                Uni.zip().unis(uni, Uni.from().<Integer>failure(new IOException("boom"))).asPair()
    ...
    public void testWithNever() {
        Uni<Tuple3<Integer, Integer, Object>> tuple = Uni.zip().unis(Uni.from().value(1),
                Uni.of(2), Uni.from().nothing()).asTuple();
    ...
    public void testTerminationJoin() {
        Uni<Void> uni = Uni.zip().unis(Uni.of(1), Uni.of("hello")).asPair().ignore().andContinueWithNull();
