...
public abstract class Tuples {
    ...
    public static <T1, T2, T3> Tuple3<T1, T2, T3> tuple3(List<?> list) {
        if (nonNull(list, "list").size() != 3) {
            throw new IllegalArgumentException("Cannot create a tuple from the given list, size must be 3 (actual size is " + nonNull(list, "list").size() + ")");
        }
    ...
    public static <T1, T2, T3, T4> Tuple4<T1, T2, T3, T4> tuple4(List<?> list) {
        if (nonNull(list, "list").size() != 4) {
            throw new IllegalArgumentException("Cannot create a tuple from the given list, size must be 4 (actual size is " + nonNull(list, "list").size() + ")");
        }
    ...
    public static <T1, T2, T3, T4, T5> Tuple5<T1, T2, T3, T4, T5> tuple5(List<?> list) {
        if (nonNull(list, "list").size() != 5) {
            throw new IllegalArgumentException("Cannot create a tuple from the given list, size must be 5 (actual size is " + nonNull(list, "list").size() + ")");
        }
